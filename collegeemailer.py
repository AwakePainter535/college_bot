...
while i >= x:
    ...
    time.sleep(0.3)
    pyautogui.write(emails[i][0]) #write name into receipient field
    time.sleep(0.3)
    pyautogui.click(x,y values from position.py) #click on subject
    time.sleep(0.3)
    pyautogui.write("Attending ") #Writing on subject line
    pyautogui.write(names[i][0]) #Writing on subject line
    ...
    pyautogui.write(names[i][0]) #Writing on email body
